...
mod camera_test {
    ...
    fn render_a_scene() {
        ...
        let mut floor = Sphere::with_material(wall_material.clone());
        floor = floor.set_transform(Matrix::identity().scale(10.0, 0.01, 10.0));

        // left wall
        let mut left_wall = Sphere::with_material(wall_material.clone());
        left_wall = left_wall.set_transform(
            Matrix::identity()
        ...
        // right wall
        let mut right_wall = Sphere::with_material(wall_material.clone());
        right_wall = right_wall.set_transform(
            Matrix::identity()
                .scale(10.0, 0.01, 10.0)
                .rotate_x(FRAC_PI_2)
                .rotate_y(FRAC_PI_4)
        ...
        let mut middle = Sphere::with_material(middle_material);
        middle = middle.set_transform(Matrix::identity().translate(-0.5, 1.0, 0.5));

        //smaller right sphere
        let mut right_material = Material::with_color(color(0.5, 1.0, 0.1));
        right_material.diffuse = 0.7;
        right_material.specular = 0.3;
        let mut right = Sphere::with_material(right_material);
        right = right.set_transform(
            Matrix::identity()
                .scale(0.5, 0.5, 0.5)
        ...
        let mut left = Sphere::with_material(left_material);
        left = left.set_transform(
            Matrix::identity()
